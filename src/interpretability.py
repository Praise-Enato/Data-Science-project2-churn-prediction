...
def logreg_importance():
    ...
    pre, lr = pipe.named_steps["pre"], pipe.named_steps["model"]
    ...
    plt.title("LogReg global importance (|coef × std|)")
    plt.tight_layout()
...
def shap_trees(which="rf"):
    ...
    feat_names = pre.get_feature_names_out()
    plt.figure()
    shap.summary_plot(shap_values, features=Xs_pre, feature_names=feat_names, plot_type="bar", show=False)
